...
class BirdImageDataset(Dataset):
    def __init__(self, root_dir, image_size=256, augment=True):
        ...
        Args:
            ...
            image_size (int): Target size for resized images.
    ...
    def _extract_label(self, path):
        ...
        try:
            label = int(folder_name.split('.')[0])  # Extract number prefix
        except ValueError:
            raise ValueError(f"Invalid folder format: {folder_name}. Expected 'XXX.ClassName'.")
        return label

    def _build_transform(self):
        """Define transformation pipeline including augmentations."""
        base_transforms = [
            transforms.CenterCrop(self.image_size),  # Crop to square
            transforms.Resize((self.image_size, self.image_size)),  # Resize to target size
            transforms.ToTensor(),
        ]
